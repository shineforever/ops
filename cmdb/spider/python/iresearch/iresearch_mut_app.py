...
# output file path
app_traffic_file_path = '../../result/mut_app_traffic.txt'

# to keep track of running state
mut_app_context = '../../result/mut_app_context.txt'
app_cur_month = '2014-12'
app_cur_page = 1
app_month_flag = 0
app_page_flag = 0

# pattern of category and subcategory
p_category = re.compile(r'\?app_category=')
...
headers = [
    ...
    ('Pragma','no-cache'),
    ('Referer','http://mut.itracker.cn:8081/AppRanking.aspx?navi=true&ranking_type=0')
]
...
def get_app_traffic():
    ...
    init_app_traffic()
    url = r'http://mut.itracker.cn:8081/AppRanking.aspx?navi=true&ranking_type=0'
    ...
    for i in range(len(months)):
        ...
        if app_month_flag == 0:
            continue
        try:
            result = get_app_traffic_by_month(months[i], html)
            if result == False:
                return
        except Exception, e:
            print e
...
def get_app_traffic_by_month(month, html):
    ...
    if app_page_flag == 1:
        get_app_traffic_detail(month, html_response, False)
...
def get_app_traffic_detail(month, html, totalPageFlag = False):
    fw = open(app_traffic_file_path, 'a')
    html = unicode(html, 'utf8')
    htmlpq = pq(html)
    data = htmlpq('#ctl00_cphPage_TrendChart1_HidChartData').val()
    dataJson = json.loads(data)
    appNames = dataJson[0]['xMarks']
    appCatData = htmlpq('#flowersTabcontent_10 .operation1 a')
    appCats = []
    appSubcats = []
    for i in range(len(appNames)):
        appCats.append(appCatData.eq(i).attr('app_category').replace(' ',''))
        appSubcats.append(appCatData.eq(i).attr('app_class').replace(' ',''))
    indexData = {}
    ...
    for i in range(len(appNames)):
        result = (result + tr_appname.eq(i+2)('td.operation1')('a').attr('app_name') + '\t' + appNames[i] + '\t' +
    ...
    fw.write(result)
    if totalPageFlag:
        pager = htmlpq('#ctl00_cphPage_AspNetPager1')('a')
        for i in range(len(pager)):
            if pager.eq(i).text() == '尾页':
                href = pager.eq(i).attr('href')
                print href
...
def set_app_traffic_params(JsonParam, month, page):
    ...
    JsonParam['PageStartIndex'] = str((page-1)*20 + 1)
    JsonParam['PageEndIndex'] = str((page-1)*20 + 20)
