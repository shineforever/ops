...
# output file path
appCategory_traffic_file_path = '../../result/iresearch/mut_appcategory_traffic_201506.txt'

# to keep track of running state
mut_appcategory_context = '../../result/iresearch/mut_appcategory_context.txt'
...
headers = [
    ...
    ('Pragma','no-cache'),
    ('Referer','http://mut.itracker.cn/AppRanking.aspx?navi=true&ranking_type=2')
]
...
def get_app_traffic():
    ...
    init_app_traffic()
    url = r'http://mut.itracker.cn/AppRanking.aspx?navi=true&ranking_type=2'
...
def get_appcategory_by_month(month, html):
    ...
    set_app_param_handler(JsonParam, month)
    ...
    formdata['ctl00$cphPage$QueryCondition1$timeWeek'] = '201509'
    formdata['ctl00$Hid_Order_Name'] = 'WeekMonthlyUniqueUsers'
    response = open_url(url, formdata)
    response_html = response.read()
    total_page = get_total_page(response_html)
    if app_page_flag == 1:
        get_appcategory_detail(month, response_html)    
    print 'total page: ', total_page
    set_formdata_for_page(response_html)
...
def get_appcategory_detail(month, html):
    fw = open(appCategory_traffic_file_path, 'a')
    html = unicode(html, 'utf8')
    htmlpq = pq(html)
    first = htmlpq('#' + index[0]).val()
    first = re.sub('\s', '', urllib.unquote_plus(first.encode('utf8')))
    appSubcategoryNames = json.loads(first)['x']['data']
    indexData = {}
    for i in range(len(index)):
        data = htmlpq('#' + index[i]).val().encode('utf8')
        data = re.sub('\s', '', urllib.unquote_plus(data))
        dataJson = json.loads(data)
        indexData[dataJson['y'][0]['name']] = dataJson['y'][0]['data']
    result = ''
    for i in range(len(appSubcategoryNames)):
        result = (result + appSubcategoryNames[i] + '\t' +
            get_main_category(appSubcategoryNames[i], htmlpq) + '\t' +
...
def set_app_param_handler(JsonParam, month):
