...
logger.add(logger.transports.File, { filename: '../log/sofang_ershou_history.log',handleExceptions: true });
...
var resultFile = "../result/sofang/sfershou_history_"+today+"_"+start+".csv";
var resultAgentFile = "../result/sofang/sfershou_agent_history_"+today+"_"+start+".csv";
...
function processList(error,result){
    ...
    var curIdxMatch = result.uri.match(/i3(\d+)/);
    curIdx = curIdxMatch ? parseInt(curIdxMatch[1]) : 1;
    var uriobj = URL.parse(result.uri);
    matches = result.uri.match(/.*?-j3100/);
    var region = matches &&  matches[0];
    var nextUrl = region + "-i3" + (curIdx+1);
    var reg = /\/chushou\/[\d_]+\.htm/g;
    var urls = result.body.match(reg);
    if(urls && urls.length>0){
    	urls = urls.filter(function(url,i){
    	    return i&1==1 && !seen.exists("http://"+uriobj.host+url);
    	}).map(function(url){
	...
	c.queue(urls);
    if(curIdx < maxIdx){
	c.queue({uri:nextUrl,callback:processList,jQuery:false});
    }
    
    logger.info("queued jobs: %d",c.queueItemSize);
}
...
function processRegions(error,result,$){
    ...
    urlListByPrice.forEach(function(url){
    	c.queue({uri:url,callback:processList,jQuery:false});
    });
    logger.info("queued jobs: %d",c.queueItemSize);
}

function getUrlByPrice(url) {
	var result = [];
	if(!cityPriceStepMap[url]) {
		result.push(url.slice(0, url.length-1)+"-i31");
		return result;
	}
	...
	while(low < highprice) {
		...
		result.push("{0}/house/a21-c2{1}-d2{2}-j3100-i31".format(domain,low, high));
		low = high;
	}
	return result;
}
...
cityurls.slice(start,start+len).forEach(function(u){
    u = u+'/house/a21-j3100/';
    c.queue({uri:u,callback:processRegions});
});
