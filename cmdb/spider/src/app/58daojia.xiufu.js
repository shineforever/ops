...
logger.add(godotTransport, {godot:client, service:"58daojia.xiufu"});

logger.add(logger.transports.File, { filename: '../../log/daojia.xiufu.log' ,logstash:true,level:'info',handleExceptions:true});
logger.cli();
...
function App() {
    ...
    this.appointmentDay = moment().add(3, "d").format("YYYY-MM-DD") + " 19:00:00";
    this.programmeStartTime = new Date().getTime();
    this.resultDir = "../../result/app/58daojia.xiufu/";
    this.resultFile = "58daojia.xiufu." + this.today + ".csv";
...
App.prototype.run = function() {
    for(var i = 0; i < this.cityids.length; i++) {
        ...
        ++this.cityLock;
        this.crawler.queue({
            uri:util.format("http://jzt2.58.com/api/v23/meijiexiufu/xiufu?r=%s&cityid=%s", App.prototype.random(), cityid),
            cityid:cityid,
            jQuery:true,
            callback:function(error, result, $) {
                ...
                for(var i = 0; i < list.length; i++) {
                    that.structuredTasks[cityid].meijiaIds.push(list.eq(i).find("input.xfid").attr("value"));
                }
                logger.info("Got %s meijiaIds for city %s", list.length, cityid);
                --that.cityLock;
                if(that.cityLock == 0) {
                    setTimeout(that.generateTasks, 0);
                }
