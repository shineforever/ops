...
var Dealer = function(){
    ...
    this.resultFile = "suning_wm_"+moment().format("YYYY-MM-DD")+".txt";
    this.done = {};
...
Dealer.prototype.start = function(){
	...
	this.wgetList(["http://list.suning.com/0-244006-0-0-0-9017.html"]);
}
...
Dealer.prototype.processItem = function(err,result,$){
	...
	var detail = {
		"title":titlestr,"proid":proid,	"brand":dealstr(brand)||"N/A", "model":dealstr(model)||"N/A",
		"type":dealstr(type)||"N/A", "onlinedate":dealstr(onlinedate)||"N/A","url":result.uri,
...
//	logger.info(detail);
	...
	//priceurl = "http://www.suning.com/webapp/wcs/stores/ItemPrice/000000000126029004__9017_10106_1.html";
	c.queue({
...
//	var prePrice = result.body.match(/"promotionPrice":"([^"]+)"/)[1];
	...
	logger.info(refPrice);
	logger.info(dealEmpty(refPrice));	
...
Dealer.prototype.processShopName=function(err,result,$){
	...
	detail["shopname"]=dealstr(shop)||"N/A";
...
Dealer.prototype.processReview=function(err,result,$){
	...
	if( result.body.slice(0,7) == "satisfy"){
		...
		logger.info("reviewCount err %d", detail["proid"]);
	}
