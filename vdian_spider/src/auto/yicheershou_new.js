...
ershou.prototype.init = function(){
    ...
    var len = Number(arguments[1])||brands.length;
    brands = brands.slice(start,start+len);
...
ershou.prototype.wgetList = function(){
	var task = that.tasks.shift();
	if(!task) {
		console.log("[INFO] Job done.");
		return;
	}
	console.log("[INFO] Working on: {0} {1}".format(task.name, task.page));
	console.log("[INFO] pages left: ", that.tasks.length);
	that.detailTasks = [];
	that.crawler.queue({
		uri:"http://www.taoche.com" + task.path,
		task:task,
		callback:function(error, result, $) {
			if(!result) {
				console.log("[ERROR] proceed to next page task.");
				setTimeout(that.wgetList, 1000);
				return;
			}
			var task = result.options.task;
			if(error || !$) {
				console.log("[ERROR] Error {0} {1}".format(task.name, task.page));
				setTimeout(that.wgetList, 1000);
				return;
			}
		    ...
		    if(task.page == 1) {
		    	...
		    	if(nextPage){
				    var totalPage = Number(nextPage.prev().text());
				    for(var page = 2; page <= totalPage; page++) {
				    	that.tasks.push({path:task.path,name:task.name,page:page});
				    }
				}
		    }
		    that.wgetDetail();
		    //setTimeout(that.wgetDetail, 1000);
		}
	});
}
...
ershou.prototype.wgetDetail = function() {
	...
	that.crawler.queue({
		...
		callback:function(error, result, $) {
			if(!result) {
				console.log("[ERROR] proceed to next detail task.");
				setTimeout(that.wgetDetail, 1000);
				return;
			}
			var detailTask = result.options.detailTask;
			if(error || !$) {
				console.log("[ERROR] Error ", result.uri);
				setTimeout(that.wgetDetail, 1000);
				return;
			}
			if(result.body.match(/商家车源/)) {
				detailTask.source = "商家车源";
				detailTask.dealerName = $(".cycsrzbox h3 a").text().trim();
				detailTask.dealerUrl = $(".cycsrzbox h3 a").attr("href");
			} else if(result.body.match(/个人车源/)) {
				detailTask.source = "个人车源";
				detailTask.dealerName = "N/A";
				detailTask.dealerUrl = "N/A";
			}
			fs.appendFileSync(that.resultDir+that.resultFile, [
					detailTask.uri,detailTask.brand,detailTask.model,detailTask.config,
					detailTask.price,detailTask.km,detailTask.year,detailTask.city,
					detailTask.source,detailTask.dealerName,detailTask.dealerUrl
				].join("\t")+"\n");
			console.log("[INFO] Got ", detailTask.uri);
			setTimeout(that.wgetDetail, 1000);
		}	
	})
}
