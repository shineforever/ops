...
logger.add(logger.transports.File, { filename: '../../log/ganji.rent.log',logstash:true,level:'info',handleExceptions:true});
if(env === 'production'){
...
logger.add(godotTransport, {godot:client, service:"gj.rent"});
...
Rent.prototype.wgetList = function(t){
    ...
    var opt = null;
    if(t.class == '1') {
        opt = {uri:"http://"+t.cityPinyin+".ganji.com/fang1/o"+t.pn+"/",t:t};
    } else {
        opt = {uri:"http://"+t.cityPinyin+".ganji.com/fang1/"+t.regionPinyin+"/o"+t.pn+"/",t:t};
    }
...
Rent.prototype.processList = function(result,$){
    ...
    ,len = $("ul.list-style1 li div.list-mod4").length
    ...
    $("ul.list-style1 li div.list-mod4").each(function(){
        var turl = $("div.info-title>a",this).attr("href")
        , pubDate = $("p.list-word",this).contents().last().text()
        , member = $("div.list-mod3 p.gj-bang-box span",this).length
	...
	var record = [mid,mname,murl,mq,t.cityName,"租房",t.districtName,t.regionName,turl,tt,tid,'gj',new Date(),new Date()];
